MODULE NeumannJumpConformal_Primal
...
CONTAINS
  ...
  !===========================================================================!
  SUBROUTINE NeumannJumpConf_Primal(Aloc, Uloc, F, ele1, ele2, edg, iFace1, iFace2, iBC, data)
  !===========================================================================!
    ...
    INTEGER                 , INTENT(IN)    :: iFace1, iFace2, iBC
    !--------------------------------------------------------------
    type(Element) :: ele
    !-------------------------
    REAL*8, DIMENSION(data%nDim,data%nDim) :: L_q
    REAL*8, DIMENSION(data%nDim+1)         :: Exa1_q, exa2_q
    REAL*8, DIMENSION(data%nDim)           :: N, GSF_i, GSF_j, xq, x_q
    !--------------------------------------------------------------------------------
    REAL*8 :: L, wq, hN_q
    ...
    nDim = Data%nDim ; nNodesPerElement = ele1%nNodesPerElement ; nVar = 1
    ...
    DO iq = 1, nQuad
       ...
       IF ( icas /= 0 ) THEN
          CALL ExactSolJumpt(ele1%L, exa1_q, x_q,Data%T, ele, nDim)
          CALL ExactSolJumpt(ele2%L, exa2_q, x_q,Data%T, ele, nDim)
          hN_q = -DOT_PRODUCT(ele2%L(1,1)*exa2_q(1:nDim) - ele2%L(1,1)*exa1_q(1:nDim),N)
       ELSE
          hN_q = Data%BCVal(iBC)
       END IF
  ...
  END SUBROUTINE NeumannJumpConf_Primal
  !===========================================================================

END MODULE NeumannJumpConformal_Primal
